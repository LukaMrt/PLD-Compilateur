...
{
public:
    Multiply(Block *block, Type type, std::string destination, std::string left, std::string right)
    ...
    void debug(std::ostream &output) const override { output << "  Multiply      " << destination << " = " << left << " * " << right << "\n"; }
