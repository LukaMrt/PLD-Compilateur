...
{
public:
    BitwiseAnd(Block *block, Type type, std::string destination, std::string left, std::string right)
    ...
    void debug(std::ostream &output) const override { output << "  BitwiseAnd    " << destination << " = " << left << " & " << right << "\n"; }
