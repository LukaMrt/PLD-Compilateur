...
{
public:
    Modulo(Block *block, Type type, std::string destination, std::string left, std::string right)
    ...
    void debug(std::ostream &output) const override { output << "  Modulo        " << destination << " = " << left << " % " << right << "\n"; }
