...
{
public:
    Greater(Block *block, Type type, std::string destination, std::string left, std::string right)
    ...
    void debug(std::ostream &output) const override { output << "  Greater    " << destination << " = " << left << " > " << right << "\n"; }
